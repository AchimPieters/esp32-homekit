    ...
    switch(src -> format) {
        ...
        case homekit_format_tlv: {
            if (src -> is_static) {
                ...
                for (tlv_t * v=src -> tlv_values -> head
                     v
                     v=v -> next) {
...
void homekit_accessories_clear_notify_callbacks(
    ...
    for (homekit_accessory_t ** accessory_it=accessories
         * accessory_it
         accessory_it + +) {
        homekit_accessory_t * accessory = *accessory_it

        for (homekit_service_t ** service_it=accessory -> services
             * service_it
             service_it + +) {
            homekit_service_t * service = *service_it

            for (homekit_characteristic_t ** ch_it=service -> characteristics
                 * ch_it
                 ch_it + +) {
