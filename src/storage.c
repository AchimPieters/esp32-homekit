...
int homekit_storage_init() {
    ...
    data_partition = esp_partition_find_first(
        ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_DATA_HOMEKIT, "homekit")
    if (!data_partition) {
    ...
    if (strncmp(magic, magic1, sizeof(magic1))) {
        ...
        if (error != ESP_OK) {
            ERROR("Failed to erase HomeKit storage: %s (0x%x)",
...
int homekit_storage_add_pairing(const char * device_id, const ed25519_key * device_key, byte permissions) {
    ...
    if (next_block_idx == -1) {
        ERROR(
            "Failed to write pairing info to HomeKit storage: max number of pairings")
        return -2
    }
    ...
    for (int i=0
        ...
        if (!strncmp(data.device_id, device_id, sizeof(data.device_id))) {
            ...
            if (homekit_storage_write(PAIRINGS_OFFSET + sizeof(data)*i, (byte * )&data, sizeof(data))) {
                        ERROR(
                            "Failed to update pairing: error erasing old record from HomeKit storage")
                        return -2
            }
